...
import (
	usersdb "github.com/ronanzindev/restapiauth/datasouce/mysql/users_db"
	"github.com/ronanzindev/restapiauth/utils/errors"
)
...
func (user *User) GetByEmail() *errors.RestErr {
	stmt, err := usersdb.Client.Prepare(queryGetUserByEmail)
	if err != nil {
...
func (user *User) GetById() *errors.RestErr {
	stmt, err := usersdb.Client.Prepare(queryGetUserById)
	if err != nil {
		return errors.NewInternalServerError("database error")
	}
	defer stmt.Close()
